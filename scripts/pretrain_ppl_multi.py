...
def train(cfg):
    ...
    for epoch_idx in range(start_epoch, epochs):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader_RoI):
            ...
            # roi_conv = conv(img_roi_feat) # [48,512,7,7]
            # cls_token = text_encoder.get_cls_token(roi_conv)  # [48,512]
            # text_embed, loss_prompt = text_encoder(cls_token, roi_conv) # [48,3,512]

            # contrastive matching (含 feature 对齐过程)
            loss_contrastive = image_encoder.roi_contrastive_matching(img_roi_feat, text_embed)
            # print("contrastive loss " + str(loss_contrastive))
            # print("loss prompt " + str(loss_prompt))
