...
# from PPL.prompt_learner import PromptLearner
...
from lib.encoder_pretrain.ppl.ppl_text_encoder_bbox import PPLTextEncoderBbox
from lib.encoder_pretrain.ppl.mono3d_encoder import ResnetEncoder


def build_class_names_prompts(bbox):
    bbox = bbox.round().int()
    bbox_list = [
        [
            f"car at ({x1}, {x2}, {y1}, {y2})"
            for (x1, x2, y1, y2) in frame.cpu().tolist()
        ]
        for frame in bbox
    ]
...
def train(cfg):
    ...
    text_encoder = PPLTextEncoderBbox().to('cuda')
    ...
    for epoch_idx in range(start_epoch, epochs):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader_RoI):
            ...
            # text modality
            roi_text_prompt_list = build_class_names_prompts(batch_input['bbox2d'])
            cls_token = text_encoder.get_cls_token(img_feat)
            text_embed, loss_prompt = text_encoder(cls_token, img_feat, roi_text_prompt_list) # text_embed: [8, 3, 512]

            # contrastive matching (含 feature 对齐过程)
            loss_contrastive = image_encoder.roi_contrastive_matching(img_feat, text_embed)
