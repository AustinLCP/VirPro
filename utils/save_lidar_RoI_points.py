...
def obtain_cluster_RoI_points_for_raw_kitti(l_3d, l_2d, seg_bbox_path, seg_mask_path, file_ind, P2):
    ...
    for index, b in enumerate(bbox2d):
        ...
        ind = obj_mask[l_2d[:, 1], l_2d[:, 0]].astype(bool)
...
if __name__ == '__main__':
    ...
    velo_list = np.char.replace(velo_list, '\\', '/')
    calib_cam_to_cam_list = np.char.replace(calib_cam_to_cam_list, '\\', '/')
    calib_velo_to_cam_list = np.char.replace(calib_velo_to_cam_list, '\\', '/')
