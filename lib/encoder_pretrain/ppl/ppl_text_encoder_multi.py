...
class PPLTextEncoderMulti(nn.Module):
    ...
    def __init__(self, class_names, n_RoI=3):
        ...
        for i in range(self.n_RoI):
            prompt_learner = PromptLearnerMulti(self.class_names, self.emb_dim).to('cuda')
    ...
    def forward(self, cls_token, visual_embeddings):
        ...
        for prompt_learner in self.prompt_learner_list:
            ...
            n_prompt = text_prompt.shape[0]

            text_embeds = self.text_encoder(text_prompt, tokenized_prompts).expand(B, -1, -1)  # (B, prompt_bsz * n_cls, C)
            text_embeddings_list.append(text_embeds)

            nc_text_embeds = self.text_encoder(nc_prompts, nc_tokenized_prompts).view(len(self.class_names), n_prompt, -1).expand(B, -1, -1, -1)  # (B, prompt_bsz * n_cls, C)
        ...
        text_embeddings = text_embeddings.reshape(text_embeddings.size(0), -1, text_embeddings.size(3)) # (B, n_roi*prompt_bsz*n_cls, C) n_cls = 1,
        nc_text_embeddings = nc_text_embeddings.mean(dim=[2]) # (B, n_roi, prompt_bsz, C)
        ...
        if self.with_prompt_encoder:
            # μ=aug_text_emb
            aug_text_emb = self.text_decoder(text_embeddings) # prompt_encoder = text_decoder
        ...
        prompt_embeddings = self.reparameterize(aug_text_emb, text_st_dev) # (B, n_cls*n_roi, C) n_cls = 1, n_component maxpooled
    ...
    def reparameterize(self, mu, logvar, n_components=8):
        ...
        for i in range(n_components):
            ...
            eps = torch.randn_like(std)  # (B, n_cls, prompt_bsz, C)
        ...
        # max pooling
        probs = probs.mean(dim=1) # (B, n_roi, C)
        return probs
